$(function() {
	...
	$("#queryOrderBtn").click(function() {
		$('#oderhistTable').datagrid({url:"/shipment/entityPick/list",queryParams:{
			orderId : $('#orderId').val(),
			startDate : $("#startDate").datebox("getValue"),
			endDate : $("#endDate").datebox("getValue"),
			status : $("#status").val(),
...
function saveOderList() {
	...
	if (orderStr != "") {
		$.post("/shipment/entityPick/saveOrder", {
			"idList" : idStr,
			"orderList" : orderStr,
...
function ajaxFileUpload(){
	...
	$.ajaxFileUpload({
		url : '/shipment/entityPick/upload?companyId=' + $("#entityId").val(),
		secureuri : false,
