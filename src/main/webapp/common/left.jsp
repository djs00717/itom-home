...
<sec:authorize ifAllGranted="omsOrderFileUpload">
    ...
    <ul>
        ...
        <li><a href="<c:url value='/order/fileUploadNew'/>">手工订单导入(新版)</a></li>
    </ul>
</sec:authorize>

    <sec:authorize ifAllGranted="omsZfbFileUpload">
        <h1>支付宝订单导入</h1>
        <ul>
            <li><a href="<c:url value='/zfb/fileUpload'/>">支付宝订单导入</a></li>
        </ul>
    </sec:authorize>

    <sec:authorize ifAnyGranted="omsOrderPreprocess,omsOrderPreprocessCreditcards,
	omsOrderPreprocessYhd,omsOrderPreprocessPOLICY,omsOrderPreprocessBiStatistics,omsOrderPreprocessTCPP,omsOrderPreprocessOther">
    <h1>前置订单处理 </h1>
    <ul>
        <sec:authorize ifAllGranted="omsOrderPreprocess">
            ...
            <li><a href="<c:url value='/orderPreprocessing/index/SUNING'/>">苏宁</a></li>
            <!--
        ...
        <sec:authorize ifAllGranted="omsOrderPreprocessOther">
        	<li><a href="<c:url value='/orderPreprocessing/index/QQPP'/>">其它商城平台</a></li>
		</sec:authorize>
		
		<sec:authorize ifAllGranted="omsOrderPreprocessTCPP">
        	<li><a href="<c:url value='/orderPreprocessing/index/TECENT_PP'/>">拍拍</a></li>
		</sec:authorize>

		<sec:authorize ifAllGranted="omsOrderPreprocessCreditcards">
            <li><a href="<c:url value='/orderPreprocessing/index/DANGDANG'/>">冷链订单</a></li>
        </sec:authorize>
		...
		<sec:authorize ifAllGranted="omsOrderPreprocess">
			<li><a href="<c:url value='/orderPreprocessing/index/DANGDANGAY'/>">当当网</a></li>
			<%-- <li><a href="<c:url value='/orderPreprocessing/index/CHAMA'/>">茶马订单</a></li> --%>
		</sec:authorize>
		
		<sec:authorize ifAllGranted="omsOrderPreprocessBiStatistics">
            <li><a href="<c:url value='/orderPreprocessing/biStatistics'/>">BI统计订单</a></li>
        </sec:authorize>
    </ul>
</sec:authorize>
...
-->
<sec:authorize ifAllGranted="omsOrderProcess,omsUnionpayRefund">
    <h1>订单处理</h1>
    <ul>
        <li><a href="<c:url value='/order/backorder/fileUpload'/>">订单压单导入</a></li>
        <li><a href="<c:url value='/order/backorder'/>">订单压单查询</a></li>
        <sec:authorize ifAllGranted="omsUnionpayRefund">
        	<li><a href="<c:url value='/payTransOrder/index'/>">银联支付退款</a></li>
        </sec:authorize>
    </ul>
</sec:authorize>
...
<sec:authorize ifAnyGranted="omsShipmentLogistics,omsOrderAuth,omsShipmentIndx,omsShipmentStock,omsOrderHistIndex,
	omsOrderPaymentConfirm,omsRequestCarrier,omsOrderHistIndexNew,omsShipmentWaveExport,omsShipmentWaveImport,
	omsShipmentWaveQuery,omsShipmentPick">
    <h1>发运单管理</h1>
    <ul>
        ...
        <sec:authorize ifAllGranted="omsShipmentPick">
            <li><a href="<c:url value='/shipment/entityPick/index'/>">承运商挑单(新规则)</a></li>
        </sec:authorize>
		...
		<sec:authorize ifAllGranted="omsShipmentWaveExport">
            <li><a href="<c:url value='/shipment/wave'/>">中外运导出</a></li>
        </sec:authorize>
        <sec:authorize ifAllGranted="omsShipmentWaveImport">    
			<li><a href="<c:url value='/shipmentBatch/importIndex'/>">中外运发运单导入</a></li>
		</sec:authorize>
        <sec:authorize ifAllGranted="omsShipmentWaveQuery">
        	<li><a href="<c:url value='/shipmentBatch/index'/>">发货波次查询</a></li>
        </sec:authorize>
    </ul>
</sec:authorize>

<sec:authorize ifAnyGranted="omsShipmentContract,omsPriceRule,omsWeightRule,omsAddreMaintain,omsCarrierIndex,omsSyncConfig,omsUrgentShipment">
    <h1>基础资料</h1>
    <ul>
        ...
        <sec:authorize ifAllGranted="omsSyncConfig">
			<li><a href="<c:url value='/inventory/syncConfig'/>">禁止同步商品列表</a></li>
		</sec:authorize>
		<sec:authorize ifAllGranted="omsUrgentShipment">
			<li><a href="<c:url value='/shipment/urge/feedbackIndex'/>">催送货反馈导入</a></li>
		</sec:authorize>
    </ul>
</sec:authorize>
<%--报表统计--%>
